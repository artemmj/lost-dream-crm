<template>
    <div class="perms-page">
        ...
        <!-- Табы: Пермишены / Роли -->
        <div class="perms-page__tabs">
            <button
                :class="['perms-page__tab', { active: activeTab === 'permissions' }]"
                @click="activeTab = 'permissions'"
            >
                Пермишены
            </button>
            <button
                :class="['perms-page__tab', { active: activeTab === 'roles' }]"
        ...
        <!-- ===== Вкладка ПЕРМИШЕНЫ ===== -->
        <div v-if="activeTab === 'permissions'" class="perms-page__section">
        ...
        <!-- ===== Вкладка РОЛИ ===== -->
        <div v-if="activeTab === 'roles'" class="perms-page__section">
            <div class="perms-page__toolbar">
                <input
                    v-model="roleSearch"
                    class="perms-page__search"
...
// All permissions (for role modal checkboxes)
...
/* Tabs */
.perms-page__tabs { display: flex; gap: 4px; margin-bottom: 24px; border-bottom: 2px solid #e5e7eb; }
.perms-page__tab {
    padding: 10px 20px; border: none; background: none; cursor: pointer;
    font-size: 14px; font-weight: 500; color: #6b7280; border-bottom: 2px solid transparent;
    margin-bottom: -2px; transition: all 0.2s;
}
.perms-page__tab:hover { color: #4f46e5; }
.perms-page__tab.active { color: #4f46e5; border-bottom-color: #4f46e5; }

/* Toolbar */
.perms-page__toolbar { display: flex; gap: 12px; margin-bottom: 16px; align-items: center; }
.perms-page__search {
    flex: 1; max-width: 400px; padding: 8px 12px; border: 1px solid #d1d5db;
...
.perms-page__btn--primary { background: #4f46e5; color: white; border-color: #4f46e5; }
.perms-page__btn--primary:hover { background: #4338ca; }
.perms-page__btn--primary:disabled { opacity: 0.6; cursor: not-allowed; }

/* Table */
.perms-page__table { width: 100%; border-collapse: collapse; background: white; border-radius: 8px; overflow: hidden; border: 1px solid #e5e7eb; }
.perms-page__table th {
    text-align: left; padding: 12px 16px; background: #f9fafb; font-size: 12px;
...
.perms-page__tag {
    background: #ede9fe; color: #7c3aed; padding: 2px 8px; border-radius: 4px; font-size: 12px;
}
...
.perms-page__pagination button:disabled { opacity: 0.4; cursor: not-allowed; }
...
.perms-page__modal input[type="text"],
.perms-page__modal input:not([type]) {
    width: 100%; margin-top: 4px; padding: 8px 12px; border: 1px solid #d1d5db;
    border-radius: 6px; font-size: 14px; outline: none; transition: border-color 0.2s;
}
...
/* Checkbox list for permissions in role modal */
.perms-page__checkbox-list {
    max-height: 240px; overflow-y: auto; border: 1px solid #e5e7eb;
    border-radius: 6px; padding: 8px; margin-top: 4px;
}
.perms-page__checkbox {
    display: flex !important; align-items: flex-start; gap: 8px; padding: 6px 4px;
    border-radius: 4px; cursor: pointer; font-weight: 400 !important;
}
